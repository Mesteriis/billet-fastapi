...
from core.exceptions.core_base import CoreRealtimeAuthError
...
class WSAuthenticator:
    ...
    async def get_current_user_http(
        ...
        raise CoreRealtimeAuthError("http", "Не авторизован - требуется JWT токен или API ключ")
...
async def require_auth(user_data: dict[str, Any] = Depends(authenticator.get_current_user_http)) -> dict[str, Any]:
    ...
    if not user_data["authenticated"]:
        raise CoreRealtimeAuthError("dependency", "Требуется авторизация")
    return user_data
