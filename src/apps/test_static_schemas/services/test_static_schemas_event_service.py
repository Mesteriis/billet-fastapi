"""
TestStaticSchema event service for Enterprise level.
...
logger = logging.getLogger("events.test_static_schemas")


class EventType(str, Enum):
    """TestStaticSchema event types."""
    CREATED = "teststaticschema.created"
    UPDATED = "teststaticschema.updated"
    DELETED = "teststaticschema.deleted"
    STATUS_CHANGED = "teststaticschema.status_changed"
    PRIORITY_CHANGED = "teststaticschema.priority_changed"
    TAGS_UPDATED = "teststaticschema.tags_updated"
    BULK_UPDATED = "teststaticschema.bulk_updated"
    CACHE_INVALIDATED = "teststaticschema.cache_invalidated"
    SEARCH_PERFORMED = "teststaticschema.search_performed"
...
class EventMetadata:
    ...
    source: str = "test_static_schemas_service"
    version: str = "1.0"
...
class DomainEvent(BaseModel):
    """
    Base domain event class for TestStaticSchema operations.
    
    This represents an event that occurred in the TestStaticSchema domain
    ...
    entity_type: str = Field(default="TestStaticSchema", description="Type of entity")
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "DomainEvent":
        ...
        return cls(
            ...
            entity_type=data.get("entity_type", "TestStaticSchema"),
...
class NotificationHandler(EventHandler):
    ...
    async def _send_creation_notification(self, event: DomainEvent):
        ...
        self.logger.info(f"TestStaticSchema {event.entity_id} created")
    ...
    async def _send_status_change_notification(self, event: DomainEvent):
        ...
        self.logger.info(f"TestStaticSchema {event.entity_id} status changed: {old_status} → {new_status}")
    ...
    async def _send_priority_change_notification(self, event: DomainEvent):
        ...
        self.logger.info(f"TestStaticSchema {event.entity_id} priority changed: {old_priority} → {new_priority}")
...
class TestStaticSchemaEventService:
    """
    Enterprise event service for TestStaticSchema domain events.
...
async def create_teststaticschema_created_event(
    entity_id: int,
...
async def create_teststaticschema_updated_event(
    entity_id: int,
...
def get_event_service() -> TestStaticSchemaEventService:
