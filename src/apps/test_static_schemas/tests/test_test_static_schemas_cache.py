"""
Tests for TestStaticSchema cache service.
...
from apps.test_static_schemas.services.test_static_schemas_cache_service import TestStaticSchemaCacheService
from apps.test_static_schemas.schemas.test_static_schemas_schemas import TestStaticSchemaResponse, TestStaticSchemaCreate


class TestCacheService:
    """Test cases for TestStaticSchema cache service."""

    @pytest.fixture
    def cache_service(self):
        """Create cache service instance with mocked Redis."""
        with patch('apps.test_static_schemas.services.test_static_schemas_cache_service.redis') as mock_redis:
            mock_redis.get.return_value = None
            mock_redis.set.return_value = True
            mock_redis.delete.return_value = 1
            mock_redis.exists.return_value = False
            
            service = TestStaticSchemaCacheService()
            service.redis = mock_redis
            return service

    @pytest.fixture
    def sample_teststaticschema(self):
        """Sample TestStaticSchema data for testing."""
        return TestStaticSchemaResponse(
            id=1,
            name="Test TestStaticSchema",
            created_at=datetime.now(),
            updated_at=datetime.now()
        )

    @pytest_asyncio.async_test
    async def test_get_from_cache_hit(self, cache_service, sample_teststaticschema):
        """Test successful cache hit."""
        # Mock Redis to return cached data
        cache_service.redis.get.return_value = json.dumps(sample_teststaticschema.model_dump())
        
        result = await cache_service.get("test_key")
        
        assert result is not None
        assert result["id"] == sample_teststaticschema.id
        assert result["name"] == sample_teststaticschema.name
    ...
    @pytest_asyncio.async_test
    async def test_set_in_cache(self, cache_service, sample_teststaticschema):
        """Test setting data in cache."""
        # Mock Redis set operation
        cache_service.redis.set.return_value = True
        
        await cache_service.set("test_key", sample_teststaticschema.model_dump(), ttl=300)
    ...
    @pytest_asyncio.async_test
    async def test_get_with_fallback(self, cache_service, sample_teststaticschema):
        ...
        async def fallback_func():
            return sample_teststaticschema.model_dump()
    ...
    @pytest_asyncio.async_test
    async def test_cache_expiration(self, cache_service, sample_teststaticschema):
        """Test cache expiration settings."""
        cache_service.redis.set.return_value = True
        
        # Test with custom TTL
        await cache_service.set("test_key", sample_teststaticschema.model_dump(), ttl=600)
