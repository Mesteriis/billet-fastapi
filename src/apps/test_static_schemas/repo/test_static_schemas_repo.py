"""
TestStaticSchema repository implementation for Advanced level.
...
class TestStaticSchemaRepository(EnterpriseRepository[TestStaticSchema, TestStaticSchemaCreate, TestStaticSchemaUpdate]):
    """
    Repository for TestStaticSchema entities with enterprise functionality.
    ...
    Example:
        ...
        ...     repo = TestStaticSchemaRepository(session)
        ...     
    ...
    def __init__(self, session: AsyncSession):
        """
        Initialize TestStaticSchema repository.
        
        Args:
            session: Database session for operations
        """
        super().__init__(model=TestStaticSchema, session=session)

    async def get_by_name_and_status(
        self, 
        name: str, 
        status: TestStaticSchemaStatus = TestStaticSchemaStatus.ACTIVE
    ) -> Optional[TestStaticSchema]:
        """
        Get teststaticschema by name and status.
        ...
        Returns:
            TestStaticSchema instance if found, None otherwise
            
        Example:
            >>> teststaticschema = await repo.get_by_name_and_status("Project Alpha", "active")
        """
    ...
    async def search_by_text(
        ...
        Returns:
            TestStaticSchemaList with search results and metadata
        ...
        response_items = []
        for item in items:
            response_data = TestStaticSchemaResponse.model_validate(item)
    ...
    async def complex_search(self, search_request: TestStaticSchemaSearchRequest) -> TestStaticSchemaList:
        """
        ...
        Returns:
            TestStaticSchemaList with filtered and sorted results
            
        Example:
            >>> request = TestStaticSchemaSearchRequest(
            ...     query="important",
            ...     status=["active", "draft"],
            ...     priority_gte=5,
            ...     sort_by="priority",
    ...
    async def get_by_priority_range(
        ...
        min_priority: int = 0, 
        max_priority: int = 10,
        status: Optional[TestStaticSchemaStatus] = None
    ) -> list[TestStaticSchema]:
        """
        Get teststaticschemas within priority range.
        ...
        Returns:
            List of TestStaticSchema instances
    ...
    async def get_status_counts(self) -> dict[str, int]:
        """
        Get count of teststaticschemas by status.
    ...
    async def bulk_update_status(
        self, 
        entity_ids: list[int], 
        new_status: TestStaticSchemaStatus
    ) -> int:
        """
        Bulk update status for multiple teststaticschemas.
        ...
        Example:
            ...
            >>> print(f"Updated {count} teststaticschemas")
        """
    ...
    async def soft_delete_by_status(self, status: TestStaticSchemaStatus) -> int:
        """
        Soft delete all teststaticschemas with given status.
