"""
TestAdvanced repository implementation for Advanced level.
...
from core.base.repo import AdvancedRepository
...
class TestAdvancedRepository(AdvancedRepository[TestAdvanced, TestAdvancedCreate, TestAdvancedUpdate]):
    """
    Repository for TestAdvanced entities with advanced functionality.
    
    This repository provides advanced CRUD functionality using AdvancedRepository
    which includes:
    ...
    Example:
        ...
        ...     repo = TestAdvancedRepository(session)
        ...     
    ...
    def __init__(self, session: AsyncSession):
        """
        Initialize TestAdvanced repository.
        
        Args:
            session: Database session for operations
        """
        super().__init__(model=TestAdvanced, session=session)

    async def get_by_name_and_status(
        self, 
        name: str, 
        status: TestAdvancedStatus = TestAdvancedStatus.ACTIVE
    ) -> Optional[TestAdvanced]:
        """
        Get testadvanced by name and status.
        ...
        Returns:
            TestAdvanced instance if found, None otherwise
            
        Example:
            >>> testadvanced = await repo.get_by_name_and_status("Project Alpha", "active")
        """
    ...
    async def search_by_text(
        ...
        Returns:
            TestAdvancedList with search results and metadata
        ...
        response_items = []
        for item in items:
            response_data = TestAdvancedResponse.model_validate(item)
    ...
    async def complex_search(self, search_request: TestAdvancedSearchRequest) -> TestAdvancedList:
        """
        ...
        Returns:
            TestAdvancedList with filtered and sorted results
            
        Example:
            >>> request = TestAdvancedSearchRequest(
            ...     query="important",
            ...     status=["active", "draft"],
            ...     priority_gte=5,
            ...     sort_by="priority",
    ...
    async def get_by_priority_range(
        ...
        min_priority: int = 0, 
        max_priority: int = 10,
        status: Optional[TestAdvancedStatus] = None
    ) -> list[TestAdvanced]:
        """
        Get testadvanceds within priority range.
        ...
        Returns:
            List of TestAdvanced instances
    ...
    async def get_status_counts(self) -> dict[str, int]:
        """
        Get count of testadvanceds by status.
    ...
    async def bulk_update_status(
        self, 
        entity_ids: list[int], 
        new_status: TestAdvancedStatus
    ) -> int:
        """
        Bulk update status for multiple testadvanceds.
        ...
        Example:
            ...
            >>> print(f"Updated {count} testadvanceds")
        """
    ...
    async def soft_delete_by_status(self, status: TestAdvancedStatus) -> int:
        """
        Soft delete all testadvanceds with given status.
